...
import {
  ...
  Filter, Building2, Clock, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
interface Job {
  id: number; title: string; company: string; companyLogo: string;
  location: string; region: string; salary: string; jobType: string;
  industry: string; requirements: string[]; visaSponsored: boolean;
  postedAt: string; viewCount: number; applyCount: number;
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <div className="bg-white p-2 rounded-2xl shadow-sm border border-gray-200 flex flex-col sm:flex-row items-center gap-2">
            <div className="w-full flex-1 flex items-center pl-4">
              ...
              <input type="text" placeholder="搜索职位、公司或关键词..." value={inputValue}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {FILTER_OPTIONS.types.map(opt => (
                      <button key={opt} onClick={() => setFilters(f => ({ ...f, type: opt }))}
                        className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filters.type === opt ? 'bg-primary/10 text-primary font-medium border border-primary/20' : 'bg-gray-50 text-gray-600 border border-gray-200 hover:bg-gray-100'}`}>
                        {opt}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <select value={filters.region} onChange={(e) => setFilters(f => ({ ...f, region: e.target.value }))}
                    ...
                    {FILTER_OPTIONS.regions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                </div>
                <div className="space-y-3">
          ...
          <div className="w-full lg:w-3/4">
            <div className="mb-4">
              <p className="text-gray-500 text-sm">共找到 <span className="font-bold text-deep">{total}</span> 个职位</p>
            </div>
            {isLoading ? (
                  ...
                  <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm animate-pulse">
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex gap-2"><div className="h-6 bg-gray-200 rounded w-16" /><div className="h-6 bg-gray-200 rounded w-20" /></div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-4">
                {jobs.map((job, index) => (
                  <motion.div key={job.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: index * 0.04 }} onClick={() => navigate(`/jobs/${job.id}`)}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-4">
                        ...
                        <div>
                          <h3 className="text-lg font-bold text-deep group-hover:text-primary transition-colors mb-1">{job.title}</h3>
                          <div className="flex flex-wrap items-center text-gray-500 text-sm mb-3 gap-x-4 gap-y-1">
                            ...
                            {job.salary && <span className="flex items-center text-green-600 font-medium"><DollarSign className="w-4 h-4 mr-1" />{job.salary}</span>}
                          </div>
                          <div className="flex flex-wrap gap-2">
                            ...
                            {(job.requirements || []).slice(0, 2).map((req, i) => (
                              <span key={i} className="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-md text-xs">{req}</span>
                            ))}
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-end space-y-4 ml-4 shrink-0">
                        ...
                        <span className="flex items-center text-xs text-gray-400"><Clock className="w-3 h-3 mr-1" />{formatDate(job.postedAt)}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="mt-8 flex justify-center items-center space-x-2">
                ...
                {visiblePages().map((p, i) => p === '...'
                  ...
                  : <button key={p} onClick={() => setPage(p as number)}
                      className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors ${page === p ? 'bg-primary text-white shadow-sm' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                      {p}
                    </button>
                )}
