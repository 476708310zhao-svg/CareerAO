...
export default function Jobs() {
    ...
    const fetchJobs = async () => {
      ...
      try {
        ...
        if (dbJobs && dbJobs.length > 0) {
          // Add firestore jobs mapping to format
          const formattedDbJobs = dbJobs.map((j: any) => ({
            id: j.id,
            title: j.title || '',
            company: j.companyName || '',
            logo: j.companyName?.charAt(0) || 'C',
            location: j.location || '',
            salary: j.salary || '',
            type: j.type || '全职',
            visa: j.visa ? '支持 H1B/OPT' : '不限身份',
            tags: [],
            postedAt: '刚发布',
            isFavorite: false
          }));
          allJobs = [...formattedDbJobs, ...MOCK_JOBS];
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 pb-12">
      <SEO 
        title="找工作/热招库 (Jobs)" 
        description="海量留学生专属热招职位，支持一键筛选H1B、OPT/CPT友好公司。快速匹配适合你的全职与实习机会。" 
        keywords="找工作, 留学生职位, H1B工作, OPT实习, 北美内推" 
      />
